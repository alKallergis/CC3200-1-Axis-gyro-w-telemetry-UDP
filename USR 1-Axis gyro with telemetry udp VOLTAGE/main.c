...
Signals:
    ...
    P03-I2C SCL to ITG-3205
    P04-I2C SDA to ITG-3205
    P15-TIMERA2A
    P05-TIMERA2B
    P01-Output signal
...
unsigned int waiting,udpSendCounter,valAdc;
long int outmatch=Period_ticks-START_TICKS;//initial match value for outmatch,close to edge
unsigned int L,H,intTest;
tBoolean above,rdy,coeffrdy;
SlSockAddrIn_t  sAddr;
tBoolean withinLoop;
tBoolean insideInt;
unsigned const char itg3205addr=0x68;
unsigned const char WHO_AM_I = 0x00;
unsigned const char SMPLRT_DIV_addr=0x15;
unsigned const char SMPLRT_DIV_conf=0x00;
unsigned const char GYRO_DPLF_ADDR = 0x16;
unsigned const char GYRO_DPLF_CONF = 0x1B;  //11011 1KHZ,42HZ LPF
unsigned const char INT_CONF_ADDR = 0x17;
unsigned const char INT_CONF_CONF = 0b00100101; //LATCH_INT_EN,ITG_RDY_EN,RAW_RDY_EN
unsigned const char INT_STATUS_ADDR = 0x1A;
unsigned const char gyro_x_h=0X1D;  //reg address
...
void i2csetupAndTest(void){
    ...
    while(I2C_IF_ReadFrom(itg3205addr,&WHO_AM_I,sizeof(WHO_AM_I),i2cbuf,1)<0);   //read whoami to confirm 0x68. loop if no connection.
    sprintf(settingsbuf,"%c%c",GYRO_DPLF_ADDR,GYRO_DPLF_CONF);
    I2C_IF_Write(itg3205addr,settingsbuf,2,1);      //write the dplf configuration

    sprintf(settingsbuf,"%c%c",SMPLRT_DIV_addr,SMPLRT_DIV_conf);
    I2C_IF_Write(itg3205addr,settingsbuf,2,1);      //write the sample rate configuration

    sprintf(settingsbuf,"%c%c",INT_CONF_ADDR,INT_CONF_CONF);
    I2C_IF_Write(itg3205addr,settingsbuf,2,1);      //write the INTERRUPT configuration
...
void readI2Crates(void){
                  ...
                  _SlNonOsMainLoopTask();
    #endif
    I2C_IF_ReadFrom(itg3205addr,&INT_STATUS_ADDR,
    ...
    if(!(drdy&1)){        //no data ready
      ...
      zrate[0]=(i2cbuf[4]<<8|i2cbuf[5])/14.375-zerr;
        ...
        if (temp<TICKS_FOR_3MS){   //be careful with wrap around arithmetic. here it won't work because the timers are 24,not 32-bit
            ...
            if(coeffrdy&&above){   //if coefficients are ready &slider is above threshold,
                ...
                if(i[0]<=m){     //first part of the curve
                    indeg[0]=(i[0]-beta1)/lamda1/80-1500;}  //will this conversion to float work??
    ...
    if (TimerIntStatus(TIMERA2_BASE,true)==TIMER_CAPB_EVENT){   //  has happened
        ...
        if ((samplesB[1]-samplesB[0])<TICKS_FOR_3MS){   //be careful with wrap around arithmetic. here it won't work because the timers are 24,not 32-bit
            ...
            if (c > TICKS_FOR_CONTROL_ENABLE){     //signal more than half,assert boolean
                above=true;
                Kp=10.0*(c-93000)/100000;
                Kd=30.0*(c-93000)/100000;
            }
            else{above=false;}
        }
    }
...
void adint3(void)
{   ADCIntClear(ADC_BASE, ADC_CH_3,ADC_FIFO_FULL);
...
void main()
{
    ...
    while (1){
        ...
        //else{}   just pass latest input signal.already passed in last ta2a neg edge..
        if(udpSendCounter==transmissionPeriodPrescale){ //send data every 20*10=200ms
/*
